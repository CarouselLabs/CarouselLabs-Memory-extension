...
var geminiSearch = OPENMEMORY_SEARCH.createOrchestrator({
  fetch: async function(query, opts) {
    ...
    return (dataJson || []).map((item) => ({ id: item.id || `memory-${Date.now()}-${Math.random().toString(36).substring(2,9)}`, text: item.memory, categories: item.categories || [] }));
  },
...
var geminiBackgroundSearchHandler = null;
...
async function handleMem0Modal(sourceButtonId = null) {
  ...
  try {
    ...
    currentModalSourceButtonId = sourceButtonId;
    geminiSearch.runImmediate(message);
...
function initializeMem0Integration() {
  ...
  try {
    setupInputObserver();
    try { hookGeminiBackgroundSearchTyping(); } catch (e) {}
    injectMem0Button();
    addSendButtonListener();
